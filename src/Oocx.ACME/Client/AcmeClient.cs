...
namespace Oocx.ACME.Client
{
    public class AcmeClient : IAcmeClient
    {
        ...
        public async Task<RegistrationResponse> RegisterAsync (string termsOfServiceUri, string [] contact)
        {
            ...
            try {
                var regUri = new Uri (directory.NewRegistration);
                var registration = await PostAsync<RegistrationResponse> (regUri, request).ConfigureAwait (false);
        ...
        public async Task<AuthorizationResponse> NewDnsAuthorizationAsync (string dnsName)
        {
            ...
            return await PostAsync<AuthorizationResponse> (new Uri (directory.NewAuthorization), authorization).ConfigureAwait (false);
        }
        ...
        public async Task<CertificateResponse> NewCertificateRequestAsync (byte [] csr)
        {
            ...
            var response = await PostAsync<CertificateResponse> (new Uri (directory.NewCertificate), request).ConfigureAwait (false);
        ...
        private async Task<TResult> SendAsync<TResult> (HttpMethod method, Uri uri, object message) where TResult : class
        {
            ...
            try {
                ...
                Trace.WriteLine ("Fuck! Fuck !!! Fuu-u-uck!!!");
            ...
            //var responseContent1 = JSserializer.Deserialize<TResult> (data);
            var responseContent = JsonConvert.DeserializeObject<TResult> (responseString);
